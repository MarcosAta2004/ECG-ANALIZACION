                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="flex align-items-center list-user-action">
                                            <button class="btn btn-sm btn-icon btn-warning" data-bs-toggle="modal" data-bs-target="#editRoleModal{{ $role->id }}" title="Editar">
                                            ...
                                            <button type="button" class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal" data-bs-target="#deleteRoleModal{{ $role->id }}" title="Eliminar">
                                                <span class="btn-inner">
                                                                ...
                                                                @foreach($permisos as $permiso)
                                                                    <div class="col-md-4 mb-2">
                                                                        <div class="form-check d-block">
                                                                            <input class="form-check-input" type="checkbox" name="permissions[]" value="{{ $permiso->name }}" id="permEdit{{ $role->id }}_{{ $permiso->id }}"
                                                                            {{ $role->hasPermissionTo($permiso->name) ? 'checked' : '' }}>
                                                                            <label class="form-check-label" for="permEdit{{ $role->id }}_{{ $permiso->id }}">
                                                                                {{ $permiso->name }}
                                                                            </label>
                                                                        </div>
                                                                    </div>
                                            ...
                                            <form action="{{ route('roles.destroy', $role->id) }}" method="POST">
                                                ...
                                                <div class="modal-body">
                                                    <div class="alert alert-warning d-flex align-items-center mb-0" role="alert">
                                                        ...
                                                        <div>
                                                            ¿Estás seguro que deseas <strong>eliminar</strong> el rol <strong>{{ $role->name }}</strong>? Esta acción no se puede deshacer.
                                                        </div>
                                                    </div>
                                                </div>  
                                                <div class="modal-footer">
            ...
            <form action="{{ route('roles.store') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12 form-group">
                            <label class="form-label">Nombre del Rol <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="name" value="{{ old('name') }}" placeholder="EJ: VENDEDOR" required>
                        </div>
                        <div class="col-md-12 form-group mt-2">
                            <label class="form-label mb-2">Permisos <span class="text-danger">*</span></label>
                            <div class="row px-3">
                                @foreach($permisos as $permiso)
                                    <div class="col-md-4 mb-2">
                                        <div class="form-check d-block">
                                            <input class="form-check-input" type="checkbox" name="permissions[]" value="{{ $permiso->name }}" id="permCreate{{ $permiso->id }}"
